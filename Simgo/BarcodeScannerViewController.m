...
- (void)viewDidLoad
{
    ...
    self.viewName = @"BarcodeScannerViewController";
    ...
    NSSet *potentialDataTypes = [NSSet setWithArray:@[AVMetadataObjectTypeCode128Code]];
/*
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    {
        NSLog(@"[%@], Type:%@", recognizedObject.stringValue, recognizedObject.type);
        ...
        if ([self isValidPlugIdFound:recognizedObject.stringValue] == YES)
        {
            [Preferences setUserPlug:recognizedObject.stringValue];
            isPlugFound = YES;
            break;
        }
    }
    ...
    if(isPlugFound == YES)
    {
        //Valid PlugID found. Jump to 'Start trip' scene
        [self pushView:@"StartTripViewController" animated:YES];
        [self.session stopRunning];
    }
}

-(BOOL) isValidPlugIdFound:(NSString *)scannedValue
{
    if (scannedValue != nil && [scannedValue length] >= MIN_PLUG_ID_LENGTH && [scannedValue length] <= MAX_PLUG_ID_LENGTH && [Utility isNumeric:scannedValue] == YES)
    {
        return YES;
    }
    return NO;
